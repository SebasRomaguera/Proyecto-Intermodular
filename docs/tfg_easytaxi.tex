...
El estado actual del proyecto (versión UD1B) incluye un sistema funcional de gestión de usuarios con registro, login y perfil personalizado, así como un completo sistema de solicitud y seguimiento de viajes en tiempo real. La versión incorpora los modelos de datos \texttt{Conductor} y \texttt{Viaje}, una API REST ampliada con endpoints de viajes, asignación automática de conductores consciente del horario (permitiendo reservas futuras sin solapes) o inmediata según necesidad, seguimiento del progreso mediante un timeline animado, cálculo estimado de precios y tiempos, sistema de valoraciones, consulta de conductores disponibles en tiempo real y posibilidad de cancelación. El proyecto está configurado para despliegue en Vercel como función serverless. Las próximas fases incorporarán mejoras adicionales como la autenticación JWT.

\textbf{Palabras clave:} Aplicación web, taxi, Node.js, MongoDB, Express, JavaScript, sistema de reservas, API REST, UI adaptativa.
...
Desarrollar una aplicación web funcional que permita solicitar, reservar y gestionar taxis en Palma de Mallorca de manera rápida, segura y accesible.
...
\begin{enumerate}
    ...
    \item \textbf{Gestionar ubicaciones y puntos de interés.}
    \begin{itemize}
        \item Autocompletado de direcciones usando ubicaciones clave de Palma.
        \item Sugerencias inteligentes para agilizar la reserva.
        \item Visualización de progreso del viaje de forma clara.
    \end{itemize}
...
GET  & /api/conductores/disponibles & Consultar disponibilidad de conductores en tiempo real \\
\hline
\end{tabular}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item El usuario introduce origen, destino y modalidad (ahora mismo o reserva programada) desde la página principal. El botón de solicitud cambia dinámicamente ("Pedir taxi" o "Reservar taxi").
        \item El sistema localiza automáticamente un conductor:
        \begin{itemize}
            \item \textbf{Para viajes inmediatos:} Asigna al conductor disponible más próximo y lo marca como "ocupado".
            \item \textbf{Para reservas futuras:} Busca un conductor sin solapes de horario y reserva el servicio sin bloquear su estado actual hasta que inicie el viaje.
        \end{itemize}
...
\subsection{Fase UD1C - Autenticación y Mejoras (Planificado)}

\begin{enumerate}
    \item \textbf{Implementación de JWT:}
    \begin{itemize}
        \item Autenticación segura y sin estado.
        \item Protección de rutas de la API.
    \end{itemize}
    
    \item \textbf{Optimizaciones de la interfaz:}
    \begin{itemize}
        \item Mejoras en la accesibilidad.
        \item Nuevos componentes interactivos.
    \end{itemize}
\end{enumerate}
